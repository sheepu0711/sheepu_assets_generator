...
{0}# **************************************************************************
{0}# Auto generated by https://github.com/fluttercandies/sheepu_assets_generator
{0}# **************************************************************************
...
class Yaml {
  ...
  void write() {
    ...
    if (start > -1) {
      ...
      final String line = lines.firstWhere((String element) => element.contains(assetsStart));
      start = yamlString.indexOf(line);
    }
    final int end = yamlString.indexOf(assetsEnd);

    if (start > -1 && end > -1) {
      yamlString = yamlString.replaceRange(start, end + assetsEnd.length, newAssets);
    } else {
      final String assetsNodeS = assets.isEmpty ? '' : '\n${indent}assets:\n\n$newAssets';
      ...
      if (yaml.containsKey('flutter')) {
        ...
        if (flutter != null) {
          if (flutter.containsKey('assets')) {
            ...
            final FileSpan sourceSpan = (flutter.nodes.keys.firstWhere((dynamic element) => element is YamlNode && element.span.text == 'assets') as YamlNode?)?.span as FileSpan;
            ...
            //Empty assets
              ...
              yamlString = yamlString.replaceRange(start, sourceSpan.end.offset + ':'.length, assetsNodeS);
            }
          }
          //miss assets:
          else {
